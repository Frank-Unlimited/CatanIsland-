...
interface Props {
  resourcesChosen: number;
}

export const YearOfPlentyPanel: React.FC<Props> = ({ resourcesChosen }) => {
  ...
  const handleResourceClick = (resource: ResourceType) => {
    gameService.chooseYearOfPlentyResource(resource);
  };
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 animate-in fade-in duration-300">
      <div className="bg-slate-900 border-2 border-amber-500 rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-black text-white">🌾 丰饶卡</h3>
          <div className="text-sm text-amber-500 font-bold">
            已选择 {resourcesChosen}/2
          </div>
        </div>
        
        <p className="text-sm text-slate-400 mb-6">
          从银行选择 {2 - resourcesChosen} 张任意资源
        </p>
